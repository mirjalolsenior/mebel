...
interface StatsData {
  jamiZapchastlar: number
  jamiKeltirilgan: number
  jamiIshlatilgan: number
}

interface ZapchastlarStatsProps {
  refreshTrigger?: number
}

export function ZapchastlarStats({ refreshTrigger = 0 }: ZapchastlarStatsProps) {
  ...
  const fetchStats = useCallback(async () => {
    ...
    try {
      // Get all zapchastlar records
      const { data: allData, error } = await supabase.from("zapchastlar").select("tovar_nomi, raqami, amal_turi, miqdor, izoh")
      ...
      const newStats = {
        jamiZapchastlar: uniqueProducts.size,
        jamiKeltirilgan,
        jamiIshlatilgan,
      }
      setStats(newStats)
    } catch (error) {
      console.error("Error fetching zapchastlar stats:", error)
    } finally {
      setLoading(false)
    }
  }, [])
  ...
  useEffect(() => {
    ...
    const subscription = supabase
      .channel("zapchastlar_changes")
      .on("postgres_changes", { event: "*", schema: "public", table: "zapchastlar" }, () => {
        fetchStats()
      })
      .subscribe()
    ...
    <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
      <Card className="glass-card border-blue-500/20 bg-blue-500/5">
        <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
          <CardTitle className="text-sm font-medium text-muted-foreground">Jami zapchastlar</CardTitle>
          <Package className="h-4 w-4 text-blue-500" />
        </CardHeader>
        <CardContent>
          <div className="text-2xl font-bold text-foreground">{stats.jamiZapchastlar}</div>
          <p className="text-xs text-muted-foreground">Turli xil zapchastlar</p>
        </CardContent>
      </Card>
      ...
      <Card className="glass-card border-green-500/20 bg-green-500/5">
        ...
        <CardContent>
          ...
          <p className="text-xs text-muted-foreground">Blok zapchastlar keltirilgan</p>
        </CardContent>
      </Card>
      ...
      <Card className="glass-card border-red-500/20 bg-red-500/5">
        ...
        <CardContent>
          ...
          <p className="text-xs text-muted-foreground">Blok zapchastlar ishlatilgan</p>
        </CardContent>
      </Card>
    </div>
  )
