...
export function ZapchastlarBalance({ refreshTrigger }: { refreshTrigger: number }) {
  ...
  useEffect(() => {
    async function fetchItems() {
      ...
      try {
        ...
        // Group by: tovar_nomi + raqami (raqami bo'sh bo'lsa ham ishlaydi)
        ...
        if (rawData) {
          for (const row of rawData) {
            ...
            // oxirgi yangilanishni eng yangi sana bo'yicha saqlab boramiz
        ...
        // List + sort (oxirgi yangilanish bo'yicha)
        ...
        console.error("Error fetching zapchastlar balance:", error)
      } finally {
        setLoading(false)
      }
    }
    ...
    <Card className="glass-card">
      ...
      <CardContent>
        <div className="space-y-4">
          ...
          {items.length === 0 ? (
            <p className="text-muted-foreground text-center py-8">Zapchastlar omborda hozircha tovarlar yo'q</p>
          ) : filteredItems.length === 0 ? (
            <p className="text-muted-foreground text-center py-8">Qidiruvga mos tovarlar topilmadi</p>
          ) : (
            filteredItems.map((item) => (
              <div key={`${item.tovar_nomi}__${item.raqami || ""}`} className="p-4 rounded-lg bg-muted/10 border border-border/50">
