...
export function ZapchastlarHistory({ refreshTrigger }: { refreshTrigger: number }) {
  const [items, setItems] = useState<ZapchastItem[]>([])
  const [filteredItems, setFilteredItems] = useState<ZapchastItem[]>([])
  ...
  useEffect(() => {
    async function fetchItems() {
      ...
      try {
        ...
        console.error("Error fetching zapchastlar history:", error)
      } finally {
        setLoading(false)
      }
    }
    ...
    return (
      <Card className="glass-card p-6">
        <div className="flex items-center justify-center h-32">
          <p className="text-muted-foreground">Zapchastlar tarixi bo'sh</p>
        </div>
      </Card>
    )
  }
            ...
            <Table>
              <TableHeader>
                <TableRow>
                  ...
                  <TableHead className="text-right">Miqdor (blok)</TableHead>
