...
import { Sofa, Plus, User, Search, X } from "lucide-react"
import { supabase } from "@/lib/supabase/client"
import { useToast } from "@/hooks/use-toast"
import { PaymentPasswordDialog } from "@/components/payment-password-dialog"

interface MebelOrder {
  id: string
  mijoz_nomi: string
  mijoz_turi: string
  qancha_tovar_keltirdi: string | null
...
interface MebelListProps {
  refreshTrigger?: number
}

export function MebelList({ refreshTrigger }: MebelListProps) {
  const [orders, setOrders] = useState<MebelOrder[]>([])
  const [loading, setLoading] = useState(true)
  const [searchQuery, setSearchQuery] = useState("")
  const [selectedOrder, setSelectedOrder] = useState<MebelOrder | null>(null)
  ...
  useEffect(() => {
    async function fetchOrders() {
      try {
        const { data, error } = await supabase.from("mebel").select("*").order("sana", { ascending: false })

        if (error) throw error

        setOrders(data || [])
      } catch (error) {
        console.error("Error fetching furniture orders:", error)
      } finally {
        setLoading(false)
      }
    }
  ...
  const handlePayment = async () => {
    ...
    try {
      ...
      const { data } = await supabase.from("mebel").select("*").order("sana", { ascending: false })
      setOrders(data || [])
  ...
  const handleOpenPayment = (order: MebelOrder) => {
      ...
      <CardContent>
        <div className="space-y-4">
          <div className="relative">
            ...
            <Input
              placeholder="Mijoz nomi orqali qidirish..."
              value={searchQuery}
          ...
          {orders.length === 0 ? (
            <p className="text-muted-foreground text-center py-8">Hozircha mebel buyurtmalari yo'q</p>
          ) : orders.filter(o => o.mijoz_nomi.toLowerCase().includes(searchQuery.toLowerCase())).length === 0 ? (
            <p className="text-muted-foreground text-center py-8">Qidiruvga mos buyurtmalar topilmadi</p>
          ) : (
            orders
              .filter(o => o.mijoz_nomi.toLowerCase().includes(searchQuery.toLowerCase()))
              .map((order) => (
              <div key={order.id} className="p-4 rounded-lg bg-muted/10 border border-border/50">
                ...
                {order.qancha_tovar_keltirdi && (
                  <div className="mb-3">
                    <p className="text-sm text-muted-foreground">Tovarlar:</p>
                    <p className="text-sm font-medium text-foreground">{order.qancha_tovar_keltirdi}</p>
                  </div>
                )}
