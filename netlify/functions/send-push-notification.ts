// netlify/functions/send-push-notification.ts
...
export const handler: Handler = async (event, context) => {
  ...
  try {
    ...
    // ------------------------
    // Prepare single message payload
    // ------------------------
    const message: any = {
      notification: {
        title,
    ...
    if (token) {
      ...
      try {
        ...
        return { statusCode: 200, body: JSON.stringify({ success: true, messageId }) };
      } catch (error: any) {
        // Log failed
        if (supabase) {
          const { data: tokenData } = await supabase
    ...
    if (tokens && Array.isArray(tokens) && tokens.length > 0) {
      const messages = tokens.map((t: string) => ({ ...message, token: t }));
          ...
          for (let i = 0; i < response.responses.length; i++) {
            ...
            if (tokenData) {
              ...
              if (
                ...
                await supabase.from("fcm_tokens").update({ is_active: false }).eq("token", t);
              }
            }
          }
        }
